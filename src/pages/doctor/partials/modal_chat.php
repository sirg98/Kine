...
// Obtener todos los doctores (excluyendo al doctor actual)
$sql = "SELECT id, nombre, apellidos, 'doctor' as tipo FROM usuarios WHERE tipo = 'doctor' AND id != $doctor_id ORDER BY apellidos, nombre";
$result = mysqli_query($conn, $sql);
        ...
        <div class="flex flex-1">
            ...
            <div class="w-1/3 bg-card dark:bg-gray-700 border-r border-card p-4 overflow-y-auto rounded-l-md">
                ...
                <div class="flex mb-4 border-b border-card">
                    ...
                    <button onclick="switchTab('doctores')" class="tab-btn px-4 py-2 text-sm font-medium text-secondary hover:text-kinetic-900 border-b-2 border-transparent hover:border-kinetic-500" data-tab="doctores">
                        Doctores
                    </button>
                </div>
...
function switchTab(tab) {
    ...
    document.getElementById('listaDoctores').classList.toggle('hidden', tab !== 'doctores');
...
buscador.addEventListener('input', function() {
    ...
    document.querySelectorAll('.contacto-btn').forEach(btn => {
        ...
        btn.parentElement.style.display = nombre.includes(term) ? '' : 'none';
    });
});
...
function cargarMensajesDoctor() {
    ...
    fetch(`/src/ajax/chat_handler_doctor.php?contacto_id=${chatContactoId}&tipo=${chatTipo}`)
        .then(r => r.json())
        .then(mensajes => {
            ...
            if (mensajes.length === 0) {
                ...
                mensajes.forEach(msg => {
                    const align = msg.emisor === 'doctor' ? 'justify-end' : 'justify-start';
                    const color = msg.emisor === 'doctor' ? 'bg-blue-500 text-white' : 'bg-gray-200 dark:bg-gray-700 text-gray-900 dark:text-gray-100';
                    cont.innerHTML += `
...
if (document.getElementById('chatForm')) {
    document.getElementById('chatForm').addEventListener('submit', function(e) {
        ...
        fetch('/src/ajax/chat_handler_doctor.php', {
            method: 'POST',
