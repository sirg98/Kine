...
<div id="modalTodasCitas" class="fixed inset-0 bg-gray-600 bg-opacity-50 dark:bg-gray-900 dark:bg-opacity-50 hidden overflow-y-auto h-full w-full z-50">
    <div class="relative top-10 mx-auto p-0 border border-card w-full max-w-6xl shadow-lg rounded-md bg-blue dark:bg-gray-800 flex flex-col min-h-[750px]">
        ...
        <div class="px-8 pt-8 pb-4 border-b border-card flex items-center justify-between">
            ...
            <button type="button" onclick="closeModalTodasCitas()" class="text-gray-400 hover:text-gray-500 dark:hover:text-gray-300">
...
function openModalTodasCitas() {
    document.getElementById('modalTodasCitas').classList.remove('hidden');
    document.body.style.overflow = 'hidden';
    cargarCitasMes(currentYear, currentMonth);
}
function closeModalTodasCitas() {
    document.getElementById('modalTodasCitas').classList.add('hidden');
    document.body.style.overflow = 'auto';
}
if (document.getElementById('modalTodasCitas')) {
    document.getElementById('modalTodasCitas').addEventListener('click', function(e) {
        if (e.target === this) closeModalTodasCitas();
    });
}
...
function cargarCitasMes(year, month, selectDate = null) {
    ...
    fetch(`/src/ajax/citas_doctor_eventos.php?year=${year}&month=${month+1}`)
        .then(r => r.json())
        .then(eventos => {
            citasPorDia = {};
            eventos.forEach(ev => {
