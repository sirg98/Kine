...
if (!isset($_SESSION['id']) || $_SESSION['tipo'] !== 'doctor') {
    die('No autorizado');
}
...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    ...
    $sql = "SELECT m.*, 
            CASE 
                WHEN m.emisor_id = $doctor_id THEN 'doctor'
                ELSE 'contacto'
            END as emisor
            FROM mensajes m
            WHERE (m.emisor_id = $doctor_id AND m.receptor_id = $contacto_id)
               OR (m.emisor_id = $contacto_id AND m.receptor_id = $doctor_id)
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    mysqli_stmt_bind_param($stmt, "iis", $doctor_id, $contacto_id, $mensaje);
